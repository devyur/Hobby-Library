...
| `STEAM_API_KEY` | Required for the Connections page's Steam import (#62) — server-side only, get one free at steamcommunity.com/dev/apikey |
...
- **Connections' Steam-imported covers are stored unresized** (issue #62) — the existing cover pipeline (#37) resizes/re-encodes client-side via Canvas/`OffscreenCanvas`, which can't run in a Server Action; Steam's own CDN already serves a pre-sized (600×900) cover per game, so the importer fetches and stores that directly rather than adding a server-side image library as a new dependency. `STEAM_API_KEY` is a separate server-only secret (get one free at steamcommunity.com/dev/apikey) — it authenticates this app to Valve and has nothing to do with the imported account's own privacy; a Steam profile's "Game details" must be set to Public in *their* Steam settings for the import to see anything, and there is no way around that (including via Steam login) short of the account owner changing it themselves.
...
- **The Supabase keepalive workflow pings with the service-role key, not the anon key** (issue #61) — this was learned the hard way. An anon-key ping to a table that only grants `SELECT` to `authenticated` gets a 401, and a *rejected* request apparently does not reset Supabase's free-tier auto-pause timer — the project paused in production despite that daily ping "succeeding" for over a week straight. If the site is ever found down after a period of no logins, check `gh run list --repo devyur/Hobby-Library --workflow=supabase-keepalive.yml` first — a paused Supabase project needs to be manually resumed from its dashboard; nothing in this codebase can do that automatically.
